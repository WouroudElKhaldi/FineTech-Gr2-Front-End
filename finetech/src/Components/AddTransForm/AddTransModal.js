...
//   return (
//     <>
//       <span onClick={handleOpen} className={styles.Exit}>
//         {type === "add" ? (
...
const TransModal = ({
  ...
  const handleOpen = (rowData) => {
    setSelectedTransactionId(rowData.id); // Set the selected transaction ID
    open.setOpen(true);
  };
  ...
  const handleEditUser = async (e) => {
    ...
    try {
      const total = await apiCall({
        url: `http://localhost:4000/api/transactionss/edit`, // Use the selected transaction ID
        method: "patch",
        data: {
        ...
        headers: {
          // Move headers outside of the data object
